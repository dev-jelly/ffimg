...
과해질 때만 한 단계씩 낮춥니다. 원본 FPS는 브라우저 메타데이터로 확인하지
못하므로 원본 프레임률 유지를 약속하지 않습니다. 가장 작은 후보도 안전 상한을
넘는 아주 긴 구간은 변환을 시작하지 않고 길이를 줄이도록 안내합니다. 고급
모드는 추천값을 명시적으로 불러올 때만 수동 설정을 변경합니다.
